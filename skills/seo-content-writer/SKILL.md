...
11. **Generate Article Images with ai-mcp**

   ## SOD Brand Visual Identity

   All images must align with the **Sun of Dawn (SOD)** brand aesthetic:
   - **Color palette**: Deep burgundy/wine red (#6B1F2A), cream white, antique gold (#C9A84C), ink black, indigo blue accents
   - **Style**: Classical Chinese high-end art — cloisonné enamel, Song Dynasty ink painting, imperial court aesthetic
   - **Motifs**: Red-crowned crane (仙鹤), lotus, auspicious clouds, Five Elements symbols, jade beads
   - **Mood**: Serene, luxurious, ancient wisdom meets modern elegance
   - **Exception**: Diagrams/infographics may use a cleaner minimal style when clarity requires it

   Generate these assets for each article using the `ai-mcp` tool:

   ### A. Article Images (3–4 per article)

   - **Hero image** (article header, 16:9): Brand-consistent scene representing the article topic
   - **Diagram/infographic** (for complex concepts): Illustrative diagram with embedded labels
   - **Section illustration** (mid-article): Scene supporting a key H2 section

   For each image:
   1. Write a detailed English prompt incorporating SOD brand aesthetic
   2. Call `ai-mcp` to generate
   3. Insert with SEO-optimized `alt` attribute

   Example hero prompt:
   ```
   Five jade and obsidian healing bead bracelets arranged in a circle on deep burgundy silk,
   antique gold decorative border, red-crowned crane motif in background, classical Chinese
   imperial aesthetic, cloisonné enamel color palette — SOD brand hero image
   ```

   ### B. Video Cover (1:1 square, one per article)

   Generate one video thumbnail cover per article:
   - **Size**: 1:1 square (suitable for YouTube, Instagram, TikTok)
   - **Style**: Classical Chinese high-end aesthetic — think imperial palace art, gold foil on deep burgundy
   - **Content**: Article topic rendered as elegant Chinese-inspired visual with English title overlay
   - **Text**: Article title in elegant serif font, gold on dark background
   - **Filename**: `cover-{article-slug}.png`

   Example cover prompt:
   ```
   Square format video thumbnail: deep burgundy background with gold cloisonné border pattern,
   white red-crowned crane in center, text overlay "Healing Crystal Bracelets" in elegant
   gold serif font, classical Chinese imperial court aesthetic, ultra high-end luxury feel,
   1:1 aspect ratio
   ```

   Insert generated images into the article markdown:
   ```markdown
   ![{target keyword} — {brief description}]({generated_image_path})
   ```

   Save video cover separately (not embedded in article):
   ```
   cover saved: ./cover-{article-slug}.png
   ```
